...
//- Проітерувати його, створиши для кожного елементу масиву <div class='member'>.
    for (let simpson of simpsons) {
        ...
        block.innerText = `${simpson.name} ${simpson.surname} ${simpson.age} ${simpson.info} ${simpson.photo}`;
        block.style.margin = '25px 0';
...
// Проітерувати його, створиши для кожного елементу масиву <div class='member'>. Для кожної властивості елементу створити окремий блок, та помістити його у div.member
    for (let simpson of simpsons) {
        ...
        blockName.innerText = simpson.name;

        let blockSurname = document.createElement('div');
        blockSurname.innerText = simpson.surname

        let blockAge = document.createElement('div');
        blockAge.innerText = simpson.age;

        let blockInfo = document.createElement('div');
        blockInfo.innerText = simpson.info;

        let blockPhoto = document.createElement('div');
        blockPhoto.innerText = simpson.photo;

        block.appendChild(blockName);
        block.appendChild(blockSurname);
        block.appendChild(blockAge);
        block.appendChild(blockInfo);
        block.appendChild(blockPhoto);
        document.body.appendChild(block);
    }
...
// Приклад структири знаходиться у файлі example.png
    for (let coursesArrayElement of coursesArray) {
        ...
        blockMonth.innerText = coursesArrayElement.monthDuration;

        let blockHour = document.createElement('div');
        blockHour.innerText = coursesArrayElement.hourDuration;

        let blockModules = document.createElement('ul');
        for (let module of coursesArrayElement.modules) {
            let listItem = document.createElement('li')
            listItem.innerText = module;
            blockModules.appendChild(listItem)
        }
        mainBlock.appendChild(blockTitle);

        wrapper.appendChild(blockMonth);
        wrapper.appendChild(blockHour);
        ...
        mainBlock.appendChild(blockModules);
